    ...
    encryption_header_t* header)
{
    ...
    if (ret != 0)
    {
      ...
      TRACE_ENCLAVE("AES context nr at %lx, value = %d", (unsigned long)(&m_aescontext.nr), m_aescontext.nr);

    rks = m_aescontext.rk;
    TRACE_ENCLAVE("RKs = %lx", (unsigned long)rks);

    for(unsigned int i = 0; i < 4; i++)
    {
        fprintf(stdout, "%08x ", rks[i]);
    }
    fprintf(stdout, "\n");
