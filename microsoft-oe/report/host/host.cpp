...
static void write_address(uint64_t offset)
{
    ...
    unsigned long host_ptr = my_enc->base + offset;

    /* Call enclave function */
    if((__result = oe_ecall(enclave, OE_ECALL_GET_SGX_REPORT, (uint64_t)host_ptr, NULL)) != OE_OK)
    {
    ...
    const char* output_file)
{
   ...
   // Roundkeys
    if(encrypt)
    {
    write_address(0x36c8d0);
    write_address(0x36c8d0 + 4);
    write_address(0x36c8d0 + 8);
    write_address(0x36c8d0 + 12);
    }
