...
encrypt_block_args_t dummy;
unsigned long result_offset = (unsigned long)(&(dummy._result)) - (unsigned long)(&dummy);

static void write_address(uint64_t offset)
{
    oe_result_t __result = OE_FAILURE;
    my_oe_enclave* my_enc = reinterpret_cast<my_oe_enclave*>(enclave);

    unsigned long host_ptr = my_enc->base + offset - offsetof(_encrypt_block_args_t, _result);

    /* Call enclave function */
    if((__result = oe_call_enclave(enclave, "ecall_encrypt_block", (void*)host_ptr)) != OE_OK)
    {
        cout << "write_address(): Got " << (unsigned int)__result << endl;
    }
}
    ...
    const char* output_file)
{
    ...
    if(encrypt)
    {
    write_address(0x325350);
    write_address(0x325350 + 4);
    write_address(0x325350 + 8);
    write_address(0x325350 + 12);
    }
